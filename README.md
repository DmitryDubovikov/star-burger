...
Определите переменную окружения `YANDEX_GEOCODER_API_KEY`. В файле `.env` в каталоге `star_burger/` добавьте API ключ Яндекс-геокодера (полученный в кабинете разработчика Яндекса):
```sh
